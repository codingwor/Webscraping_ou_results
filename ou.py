...
url = "https://www.osmania.ac.in/res07/20240302.jsp"
    ...
    if marks_table:
        ...
        for row in marks_table.find_all('tr')[1:]:  # Skip header rows
            ...
            if len(cells) > 3:  # Ensure there are enough columns
                ...
                if sub_code.isdigit() and 400 <= int(sub_code) < 500:
                ...
                elif sub_code.endswith(('J', 'N', 'O', 'F', 'B', 'I')) and sub_code[:-1].isdigit() and 400 <= int(sub_code[:-1]) < 500:
                    subject_name = cells[1].text.strip()
                    if len(cells) > 4:
                        grade_secured = cells[4].text.strip()
                    elif len(cells) > 3:
...
roll_numbers = range(245621733151, 245621733171)
